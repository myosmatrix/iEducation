...
- (void) prepare {
    ...
    filenamearray = [[NSMutableArray alloc]initWithCapacity:11];
    ...
    for (int i = 0; i < NUM_VIDEO_TARGETS; ++i) {
        ...
        switch (i) {
            case 0:
                filename = @"eMeeting+.mp4";
                break;
            case 1:
                filename = @"北京台鱼骨全景程序+.mp4";
                break;
            case 2:
                filename = @"北京云计算大会+logo改.mp4";
                break;
            case 3:
                filename = @"发布会篇成片+.mp4";
                break;
            case 4:
                filename = @"互动报告厅+logo改.mp4";
                break;
            case 5:
                filename = @"互动教室—海量教学搜索功能演示.mp4";
                break;
            case 6:
                filename = @"会议篇总合成+.mp4";
                break;
...
- (void)renderFrameQCAR
{
    ...
    for (int i = 0; i < numActiveTrackables; ++i) {
        ...
        if (strcmp(imageTarget.getName(), "T2") == 0)
        {
            playerIndex = 1;
        }else if (strcmp(imageTarget.getName(), "T3") == 0)
        {
            playerIndex = 2;
        }else if (strcmp(imageTarget.getName(), "T4") == 0)
        {
            playerIndex = 3;
        }else if (strcmp(imageTarget.getName(), "T5") == 0)
        {
            playerIndex = 4;
        }else if (strcmp(imageTarget.getName(), "T6") == 0)
        {
            playerIndex = 5;
        }else if (strcmp(imageTarget.getName(), "T7") == 0)
        {
            playerIndex = 6;
        }else if (strcmp(imageTarget.getName(), "T8") == 0)
        {
            playerIndex = 7;
        }else if (strcmp(imageTarget.getName(), "T9") == 0)
        {
            playerIndex = 8;
        }else if (strcmp(imageTarget.getName(), "T10") == 0)
        {
            playerIndex = 9;
        }else if (strcmp(imageTarget.getName(), "T11") == 0)
        {
            playerIndex = 10;
        }else if (strcmp(imageTarget.getName(), "T12") == 0)
        {
            playerIndex = 11;
        }else if (strcmp(imageTarget.getName(), "T13") == 0)
        {
            playerIndex = 12;
        }else if (strcmp(imageTarget.getName(), "T14") == 0)
        {
            playerIndex = 13;
        }else if (strcmp(imageTarget.getName(), "T15") == 0)
        {
            playerIndex = 14;
        }

        if (playerIndex != lastIndex && lastIndex != 100) {
            [showingplayer release];
            [showingplayer release];
            zlytime = 0;
        }
        if (zlytime == 0) {
            zlytime = 1;
            VideoPlayerHelper * player = [self getVideoPlayerHelper:playerIndex];
            NSString * filename = [filenamearray objectAtIndex:playerIndex];
            if (NO == [player load:filename playImmediately:NO fromPosition:videoPlaybackTime[playerIndex]]) {
                NSLog(@"Failed to load media");
            }
